...
void SecurityChecksPass::markBBErr(BasicBlock *BB, ErrFlag flag,
                                   BBErrMap &bbErrMap) {
...
void SecurityChecksPass::recurMarkEdgesToErrReturn(BasicBlock *BB, int flag,
                                                   EdgeErrMap &edgeErrMap) {
  ...
  while (!EB.empty()) {
    ...
    for (BasicBlock *predBB : predecessors(TB)) {
      Instruction *TI = predBB->getTerminator();
      CFGEdge CE = std::make_pair(TI, TB);
      if ((edgeErrMap[CE] & ERR_RETURN_MASK) == (flag & ERR_RETURN_MASK))
        continue;
...
void SecurityChecksPass::markEdgesToErrReturn(BasicBlock *BB, int flag,
                                              EdgeErrMap &edgeErrMap) {
...
bool SecurityChecksPass::mayReturnErr(Function *F) {
  ...
  while (!EF.empty()) {
    ...
    for (Function::iterator b = TF->begin(), e = TF->end(); b != e; ++b) {
      BasicBlock *BB = &*b;
...
void SecurityChecksPass::checkErrReturn(Function *F, BBErrMap &bbErrMap) {
...
void SecurityChecksPass::checkErrHandle(Function *F, BBErrMap &bbErrMap) {
...
void SecurityChecksPass::identifySecurityChecks(Function *F,
  ...
  // the index of the successor of the terminator instruction. This data
  // structure may need to be promoted to SecurityChecksPass.
  ...
  // Filtering
  if (edgeErrMap.size() == 0 && ErrSelectInstSet.size() == 0)
    return;
...
void SecurityChecksPass::checkErrValueFlow(Function *F, ReturnInst *RI,
                                           std::set<Value *> &PV,
                                           BBErrMap &bbErrMap) {
  ...
  EEV.push_back(
      std::make_pair(std::make_pair((Instruction *)NULL, RI->getParent()), RV));
  ...
  while (!EEV.empty()) {
    ...
    if (SI) {
      ...
      if (isConstant(SV)) {
        ...
        EEV.push_back(std::make_pair(
            make_pair(SI->getParent()->getTerminator(), BB), SV));
      }
    ...
    if (GEP) {
      ...
      EEV.push_back(
          std::make_pair(make_pair(GEP->getParent()->getTerminator(), BB), PO));
      ...
      if (FIter != Ctx->CopyFuncs.end()) {
        if (get<1>(FIter->second) == -1) {
          ...
          if (isValueErrno(Arg, F)) {
            ...
            EEV.push_back(make_pair(
                make_pair(CaI->getParent()->getTerminator(), BB), Arg));
            continue;
          }
        }
      }
    ...
    if (UI) {
      ...
      EEV.push_back(
          std::make_pair(make_pair(UI->getParent()->getTerminator(), BB), UO));
...
#ifdef DEBUG_PRINT
    OP << "== Warning: unsupported LLVM IR:" << *V << '\n';
    assert(0);
#endif
  }
  return;
...
bool SecurityChecksPass::doInitialization(Module *M) { return false; }

bool SecurityChecksPass::doFinalization(Module *M) { return false; }
