...
private:
  ...
  void checkErrValueFlow(Function *F, ReturnInst *RI, std::set<Value *> &PV,
                         BBErrMap &bbErrMap);

  // Traverse CFG to mark all edges with error flags
  bool markAllEdgesErrFlag(Function *F, BBErrMap &bbErrMap,
                           EdgeErrMap &edgeErrMap);

  // Recursively mark all edges from the given block
  void recurMarkEdgesFromBlock(CFGEdge &CE, int flag, BBErrMap &bbErrMap,
                               EdgeErrMap &edgeErrMap);

  // Recursively mark all edges to the given block
  void recurMarkEdgesToBlock(CFGEdge &CE, int flag, BBErrMap &bbErrMap,
                             EdgeErrMap &edgeErrMap);
  ...
  void recurMarkEdgesToErrReturn(BasicBlock *BB, int flag,
                                 EdgeErrMap &edgeErrMap);
...
public:
  ...
  void identifySecurityChecks(Function *F, EdgeErrMap &edgeErrMap,
